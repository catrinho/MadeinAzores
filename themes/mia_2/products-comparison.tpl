			...
			<td width="{$width}%" class="ajax_block_product comparison_infos"><div class="product-container">
				...
				{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
					{if ($product->quantity > 0 OR $product->allow_oosp)}
						<a class="btn btn-default btn-md ajax_add_to_cart_btn btn-lg btn-default" rel="ajax_id_product_{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")}" title="{l s='Add to cart'}"><span></span>{l s='Add to cart'}</a>
					{else}
						<span class="btn btn-default btn-md">{l s='Add to cart'}</span>
					{/if}
				{else}
