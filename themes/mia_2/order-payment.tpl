...
{if !$opc}<h1 class="title_category">{l s='Choose your payment method'}</h1>{else}<h3>{l s='Choose your payment method'}</h3>{/if}
...
<div id="order-detail-content" class="table_block">
	<table id="cart_summary" class="std">
		...
		{foreach from=$products item=product name=productLoop}
			...
			{assign var='noDeletebtn btn-lg btn-default' value=1}
					...
					<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" class="alternate_item cart_item">
						...
						<td class="cart_quantity">
							...
							{else}
								...
								<div id="cart_quantity_btn btn-lg btn-default" style="float:left">
...
{if !$opc}
	<p class="cart_navigation"><a href="{$link->getPageLink('order', true, NULL, "step=2")}" title="{l s='Previous'}" class="btn btn-lg btn-default">&laquo; {l s='Previous'}</a></p>
{else}
	</div>
{/if}
</div>
