...
{if isset($attributesCombinations)}
	...
	{/foreach}
{/if}
//]]>{$img_dir}icon/ecommerce/cart.jpg
</script>
<style type="text/css">
.add-cart{
...
<div id="primary_block"  class="row">
	...
	<div class="col-lg-12">
		<!-- left infos-->
		<div id="pb-left-column" class="col-lg-5">

			<!-- description short -->
			{if $product->description_short OR $packItems|@count > 0}
			<div id="short_description_block">
				<h5>{l s='Quick Overview'}</h5>
				{if $product->description_short}
					<div id="short_description_content" class="produto-descricao rte text_justify">{$product->description_short}</div>
				{/if}
			</div>
			...
			<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
				...
				<div class="row">
		           <p>
		           	<div id="add_to_cart" class="add-cart">
			...
			{/if}
							<!-- attributes -->
				<div class="product_attributes" style>
					{if isset($groups)}
					<!-- attributes -->
					<div id="attributes">
		...
		<!-- right infos-->
		<div id="pb-right-column" class="col-lg-7">
			<div class="images-block row">
				<!-- product img-->
				<div id="image-block col-lg-12">
				{if $have_image}
					<span id="view_full_size">
						<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_leometr')}" {if $jqZoomEnabled}class="jqzoom image-responsive" alt="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_leometr')}"{else} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" {/if} id="bigpic" width="{$largeSize.width}" height="{$largeSize.height}"/>
