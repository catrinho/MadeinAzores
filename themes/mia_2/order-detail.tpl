...
<form action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
	<div>
		...
		<p class="title_block">
			<input type="submit" value="{l s='Reorder'}" name="submitReorder" class="btn btn-lg btn-default btn btn-default btn-md" />
...
{if !$is_guest}
	...
	<div id="returnOrderMessage">
		...
		<p>{l s='If you wish to return one or more products, please mark the corresponding boxes and provide an explanation for the return. Then click the btn btn-lg btn-default below.'}</p>
		<p class="textarea">
			<textarea cols="67" rows="3" name="returnText"></textarea>
		</p>
		<p class="submit">
			<input type="submit" value="{l s='Make an RMA slip'}" name="submitReturnMerchandise" class="btn btn-lg btn-default_large" />
	...
	<form action="{$link->getPageLink('order-detail', true)}" method="post" class="std" id="sendOrderMessage">
		...
		<p class="submit">
			...
			<input type="submit" class="btn btn-lg btn-default" name="submitMessage" value="{l s='Send'}"/>
		</p>
	</form>
{else}
