...
if( !class_exists("LeoBaseSource") ){
	include_once(_PS_MODULE_DIR_.'leocamerapromo/libs/BaseSource.php');	
}
include_once(_PS_MODULE_DIR_.'leocamerapromo/libs/Slideshow.php');
include_once(_PS_MODULE_DIR_.'leocamerapromo/libs/Params.php');
if (!class_exists('PhpThumbFactory')) {
    require  _PS_MODULE_DIR_.'leocamerapromo/libs/phpthumb/ThumbLib.inc.php';
}

class leocamerapromo extends Module
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Leo slide image using camera');
	...
	{
		/* Adds Module */
		if (parent::install() && $this->registerHook('DisplaySlideshow'))
		{
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 5; ++$i)
		{
			$slide = new leocamerapromoControl();
			$slide->position = $i;
			$slide->active = 1;
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new leocamerapromoControl($slide['id_slide']);
			$to_del->delete();
		}
	...
	{	
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slide = new leocamerapromoControl((int)Tools::getValue('id_slide'));
		...
		foreach ($languages as $language)
		{
			...
			/* Display image */
			if ($slide && $slide->image[$language['id_lang']])
				$this->_html .= '<img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/images/'.$slide->image[$language['id_lang']].'" width="'.($this->getParams()->get('imgwidth',940)/2).'" height="'.($this->getParams()->get('imgheight',438)/2).'" alt=""/>';
			$this->_html .= '</div>';
		}
	...
	private function _postProcess()
	{
		...
		{
			$slide = new leocamerapromoControl((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			if (!$res)
	...
	//	echo '<pre>'.print_r( $slider, 1 ); die;
		...
		$slides = LeoBaseSource::getSource( $source )
			->setModuleName('leocamerapromo',$path,$site_url )
		...
		$this->smarty->assign('leocamerapromo_slides', $slides);
		$this->smarty->assign('leocamerapromo', $slider);
		return true;
	}
	...
	public function hookDisplayTop()
	{
		...
		$this->context->controller->addCSS($this->_path.'css/leocamerapromo.css');
		...
		ob_start();
		 require( dirname(__FILE__).'/initscript.php' );		
	    $script = ob_get_contents();
	    ob_end_clean();
		
		return $this->display(__FILE__, $tpl ).$script;
	}
