...
if( !class_exists("LeoImagesSource") ){	
	...
	class LeoImagesSource extends LeoBaseSource{
		...
		public function getData( $params , $table='leocamerapromo'){
			...
			$sliders = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				...
				LEFT JOIN '._DB_PREFIX_.$table.'_slides hss ON (hs.id_leocamerapromo_slides = hss.id_'.$table.'_slides)
				LEFT JOIN '._DB_PREFIX_.$table.'_slides_lang hssl ON (hss.id_leocamerapromo_slides = hssl.id_'.$table.'_slides)
