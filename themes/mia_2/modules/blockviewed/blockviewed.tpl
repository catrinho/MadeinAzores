...
<div id="viewed-products_block_left" class="block products_block">
	<h3 class="title_block">{l s='Viewed products' mod='blockviewed'}</h3>
		...
		<ul class="products clearfix">
			{foreach from=$productsViewedObj item=viewedProduct name=myLoop}
				<li class="clearfix{if $smarty.foreach.myLoop.last} last_item{elseif $smarty.foreach.myLoop.first} first_item{else} item{/if}">
					...
					<img src="{if isset($viewedProduct->id_image) && $viewedProduct->id_image}{$link->getImageLink($viewedProduct->link_rewrite, $viewedProduct->cover, 'medium_leometr')}{else}{$img_prod_dir}{$lang_iso}-default-medium_leometr.jpg{/if}" alt="{$viewedProduct->legend|escape:html:'UTF-8'}" />
					</a>
					<div class="text_desc">
