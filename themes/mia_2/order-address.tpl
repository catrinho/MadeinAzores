  ...
  <div class="col-lg-12 col-md-12 col-sm-12">
    <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12 section-title">
        <div class="row">
        {if !$opc}<h1>{l s='Addresses'}</h1>{
        else}<h2>{l s='Addresses'}</h2>{/if}
        </div>
      </div>
    </div>
    <div class="row">
...
{if !$opc}
	...
	{if !$multi_shipping && {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="exclusive_multishipping_mode" id="multishipping_mode_box">
	...
	{/if}
<form action="{$link->getPageLink($back_order_page, true)}" method="post" role="form" class="form-horizontal">
{else}
	{if {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="address-form-multishipping">
			<div class="btn btn-lg btn-default_multishipping_mode" id="multishipping_mode_box">
				<div class="title">{l s='Multi-shipping'}</div>
				<div class="description">
...
{/if}
	<div class="addresses clearfix">
		...
		{if $addresses|@count > 1}
			...
			{else}
				<a style="margin-left: 0px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="btn btn-lg btn-default_large">{l s='Add a new address'}</a>
			{/if}
		</p>
		<div class="row">
			<ul class="address item col-lg-6 col-md-6 col-sm-6 " id="address_delivery" {if $cart->isVirtualCart()}style="display:none;"{/if}>
			</ul>
			<ul class="address col-lg-6 col-md-6 col-sm-6 alternate_item {if $cart->isVirtualCart()}span12{else}span6{/if}" id="address_invoice">
			</ul>
		</div>
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="btn-link">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
...
{if !$opc}
	<p class="cart_navigation pull-right submit">
		<input type="hidden" class="hidden" name="step" value="2" />
		<input type="hidden" name="back" value="{$back}" />
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}" title="{l s='Previous'}" class=" btn-lg btn-link">&laquo; {l s='Previous'}</a>
		<input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="btn btn-default btn-md btn-link btn-lg" />
	</p>
</form>
{else}
</div>
