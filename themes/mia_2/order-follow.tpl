...
{if isset($errorQuantity) && $errorQuantity}<p class="alert alert-danger">{l s='You do not have enough products to request another merchandise return.'}</p>{/if}
{if isset($errorMsg) && $errorMsg}
	...
	<p>
		...
		<form method="POST"  id="returnOrderMessage"/>
			...
			<input class="btn btn-lg btn-default_large" type="submit" name="submitReturnMerchandise" value="{l s='Make an RMA slip'}"/>
		</form>
	</p>
{/if}
...
{if isset($errorDetail2) && $errorDetail2}<p class="alert alert-danger">{l s='Please provide a quantity for the product you checked.'}</p>{/if}
...
<div class="block-center" id="block-history">
	...
	{else}
		<p class="alert alert-warning">{l s='You have no return merchandise authorizations.'}</p>
	{/if}
</div>
