...
{if isset($cms) && !isset($cms_category)}
	{if !$cms->active}
		...
		<div id="admin-action-cms">
			...
			<input type="submit" value="{l s='Publish'}" class="btn btn-default btn-md" onclick="submitPublishCMS('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 0, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>
			<input type="submit" value="{l s='Back'}" class="btn btn-default btn-md" onclick="submitPublishCMS('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 1, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>
			</p>
			<div class="clear" ></div>
	...
	{/if}
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 no-padding-row rte{if $content_only} content_only{/if}">
			{$cms->content}
		</div>
	</div>
