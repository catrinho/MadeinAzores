...
function displayImage(domAAroundImgThumb, no_animation)
{
	...
	{	
		var newSrc_XL = domAAroundImgThumb.attr('href');
		var newSrc_L = domAAroundImgThumb.attr('href').replace('thickbox','large');
		


		if ($('#bigpic').attr('src') != newSrc_L)
		{
			$('#bigpic').fadeOut((no_animation ? 0 : 'fast'), function(){
				$(this).attr('src', newSrc_L).show();
				$('#img-link').attr('href', newSrc_XL);
...
{
// AddQuanttiy product button
$('#ProductAddQuantity').bind('click',function () {
	var quantity = $('#quantity_wanted').val();
	quantity=++quantity;
	$('#quantity_wanted').val(quantity);
})
// SubtractQuanttiy product button
$('#ProductSubtractQuantity').bind('click',function () {
	var quantity = $('#quantity_wanted').val();
	if(quantity > 0){
			quantity=--quantity;
	}
