...
{if isset($empty)}
	<p class="alert alert-warning">{l s='Your shopping cart is empty. Buy something!'} <a href="{$base_dir}">{l s="See the Catalog's" }</a> </p>
...
{/if*}
<blockquote class="nproducts">
	<small>{l s='Your shopping cart contains:'}&nbsp;<cite id="summary_products_quantity"><b>{$productNumber}</b> {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}</cite></small>
</blockquote>
...
{*}
<div id="order-detail-content" class="table-responsive">
	<table id="cart_summary" class="table std">
		<thead>
			<tr>
				<th class="cart_product first_item hidden-phone text-left" style="
    padding-left: 0px;">{l s='Product'}</th>
				<th class="cart_description item text-center hidden-phone">{l s='Description'}</th>
				<th class="cart_unit text-center item">{l s='Unit price'}</th>
				<th class="cart_quantity text-center item">{l s='Qty'}</th>
				<th class="cart_total text-center item">{l s='Total'}</th>
				<th class="cart_delete text-right last_item">&nbsp;</th>
			</tr>
		</thead>
				...
				{foreach $customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] as $id_customization=>$customization}
					<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}" class="product_customization_for_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if} customization alternate_item {if $product@last && $customization@last && !count($gift_products)}last_item{/if}">
						...
						<td class="cart_quantity" >
							...
							{else}
								<div class="cart_quantity_btn btn-lg btn-default">
	...
	{/if}
	<div class=" col-lg-6 col-lg-push-6 col-md-6 col-md-push-6 col-sm-12 table-responsive">
	
		<table  class="table total-table std">
			<tbody>
			{if $use_taxes}
				{if $priceDisplay}
			...
			{/if}
					...
					{if $voucherAllowed}
						...
						<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
							<fieldset>
								...
								<p class="submit"><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='OK'}" class="btn btn-lg btn-default" /></p>
							</fieldset>
						</form>
						{if $displayVouchers}
				...
				<tr class="cart_total_price_final">
					{if $use_taxes}
	...
	<div class=" col-lg-6 col-lg-pull-6 col-md-6 col-md-pull-6 col-sm-6">
		<a class="btn btn-default" href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}"  title="{l s='Continue shopping'}" >&laquo; {l s='Continue shopping'}</a>
...
<p class="cart_navigation pull-right">
	{if !$opc}
	<a class="btn-lg btn-link" href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}"  title="{l s='Continue shopping'}" >&laquo; {l s='Continue shopping'}</a>
	
		<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" class="btn-lg btn-link exclusive standard-checkout" title="{l s='Next'}">{l s='Next'} &raquo;</a>
		{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
			<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1" class="btn-lg btn-link multishipping-btn btn-lg btn-default multishipping-checkout exclusive" title="{l s='Next'}">{l s='Next'} &raquo;</a>
		{/if}
	{/if}
</p>
