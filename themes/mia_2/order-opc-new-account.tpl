<div id="opc_new_account" class="opc-main-block row">
{strip}
...
{/strip}
		...
		<fieldset>
			...
			<div id="login_form_content" style="display:none;">
				...
				<div style="margin-left:40px;margin-bottom:5px;float:left;width:40%;">
					...
					<span><input type="text" id="login_email" name="email" class="form-control" /></span>
				</div>
				<div style="margin-left:40px;margin-bottom:5px;float:left;width:40%;">
					<label for="login_passwd">{l s='Password'}</label>
					<span><input type="password" id="login_passwd" name="login_passwd" class="form-control" /></span>
					<a href="{$link->getPageLink('password', true)}" class="lost_password">{l s='Forgot your password?'}</a>
				</div>
				<p class="submit">
					{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
					<input type="submit" id="SubmitLogin" name="SubmitLogin" class="btn btn-lg btn-default" value="{l s='Log in'}" />
				</p>
			</div>
		</fieldset>
	</form>
	<form action="javascript:;" method="post" id="new_account_form" class="std" autocomplete="on" autofill="on">
		<fieldset>
			<h4 id="new_account_title">{l s='New Customer'}</h4>
			<div id="opc_account_choice">
				<div class="opc_float">
					<p class="title_block">{l s='Instant Checkout'}</p>
					<p>
						<input type="btn btn-lg btn-default" class="btn btn-default btn-md_large" id="opc_guestCheckout" value="{l s='Checkout as guest'}" />
					</p>
				</div>
				...
				<div class="opc_float">
					...
					<p>
						<input type="btn btn-lg btn-default" class="btn btn-lg btn-default_large" id="opc_createAccount" value="{l s='Create an account'}" />
					</p>
				</div>
				<div class="clear"></div>
			</div>
			<div id="opc_account_form">
				...
				<div class="form-group radio required">
					<label for="gender">{l s='Title'}</label>
					{foreach from=$genders key=k item=gender}
						<input class="form-titles" type="radio" name="id_gender" id="id_gender{$gender->id_gender}" value="{$gender->id_gender}" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id_gender}checked="checked"{/if} />
				...
				<div class="form-group checkbox is_customer_param">
				...
				<div id="opc_invoice_address" class="is_customer_param">
					...
					<div id="vat_number_block_invoice" class="is_customer_param" style="display:none;">
						<div class="form-group text">
							...
							<input type="text" class="text form-control" id="vat_number_invoice" name="vat_number_invoice" value="" />
						</div>
					</div>
					<div class="form-group required text dni_invoice">
						<label for="dni">{l s='Identification number'}</label>
						<input type="text" class="text form-control" name="dni_invoice" id="dni_invoice" value="{if isset($guestInformations) && $guestInformations.dni}{$guestInformations.dni}{/if}" />
						<span class="form_info">{l s='DNI / NIF / NIE'}</span>
					</div>
					{elseif $field_name eq "firstname"}
					<div class="form-group required text">
						<label for="firstname_invoice">{l s='First name'} <sup>*</sup></label>
						<input type="text" class="text form-control" id="firstname_invoice" name="firstname_invoice" value="" />
					</div>
					{elseif $field_name eq "lastname"}
					<div class="form-group required text">
						<label for="lastname_invoice">{l s='Last name'} <sup>*</sup></label>
						<input type="text" class="text form-control" id="lastname_invoice" name="lastname_invoice" value="" />
					</div>
					{elseif $field_name eq "address1"}
					<div class="form-group required text">
						<label for="address1_invoice">{l s='Address'} <sup>*</sup></label>
						<input type="text" class="text form-control" name="address1_invoice" id="address1_invoice" value="" />
					</div>
					{elseif $field_name eq "address2"}
					<div class="form-group text is_customer_param">
						<label for="address2_invoice">{l s='Address (Line 2)'}</label>
						<input type="text" class="text form-control" name="address2_invoice" id="address2_invoice" value="" />
					</div>
					{elseif $field_name eq "postcode"}
					<div class="form-group required postcode text">
						<label for="postcode_invoice">{l s='Zip / Postal Code'} <sup>*</sup></label>
						<input type="text" class="text form-control" name="postcode_invoice" id="postcode_invoice" value="" onkeyup="$('#postcode').val($('#postcode').val().toUpperCase());" />
					</div>
					{elseif $field_name eq "city"}
					<div class="form-group required text">
						<label for="city_invoice">{l s='City'} <sup>*</sup></label>
						<input type="text" class="text form-control" name="city_invoice" id="city_invoice" value="" />
					</div>
					{elseif $field_name eq "country" || $field_name eq "Country:name"}
					<div class="form-group required select">
					...
					<div class="form-group required text">
						...
						<input type="text" class="text form-control" name="phone_invoice" id="phone_invoice" value="" />
					</div>
					<div class="form-group text is_customer_param">
						<label for="phone_mobile_invoice">{l s='Mobile phone'}</label>
						<input type="text" class="text form-control" name="phone_mobile_invoice" id="phone_mobile_invoice" value="" />
					</div>
					<input type="hidden" name="alias_invoice" id="alias_invoice" value="{l s='My Invoice address'}" />
				</div>
				{$HOOK_CREATE_ACCOUNT_FORM}
				<div class="form-group submit">
					<input type="submit" class="btn btn-default btn-md btn btn-lg btn-default" name="submitAccount" id="submitAccount" value="{l s='Save'}" />
				</div>
				<p style="display: none;" id="opc_account_saved">
					{l s='Account information saved successfully'}
				</p>
				<div class="form-group required opc-required" style="clear: both;">
