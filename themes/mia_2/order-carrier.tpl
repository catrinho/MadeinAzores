...
{if !$opc}
	<div id="carrier_area">
{else}
	<div id="carrier_area" class="opc-main-block row">
{/if}
...
{else}
	<div id="opc_delivery_methods" class="opc-main-block  col-lg-12 col-md-12 col-sm-12 no-padding-row">
...
{else}
	...
	{if isset($isVirtualCart) && $isVirtualCart}
		<p class="alert alert-warning">{l s='No carrier needed for this order'}</p>
	{else}
			...
			{foreach $option_list as $key => $option}
				<div class="delivery_option {if ($option@index % 2)}alternate_{/if}item">
					...
					<label for="delivery_option_{$id_address}_{$option@index}">
						<table class="resume table table-no-stroke">
							<tr>
								<td>
						...
						<table class="table table-no-stroke delivery_option_carrier {if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key}selected{/if} {if $option.unique_carrier}not-displayable{/if}">
							{foreach $option.carrier_list as $carrier}
							<tr>
								{if !$option.unique_carrier}
								<td class="first_item">
...
{if $conditions AND $cms_id}
	...
	<script type="text/javascript">$('.iframe').magnificPopup({
  		type: 'iframe'
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		{if !$is_guest}
			{if $back}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&back={$back}&multi-shipping={$multi_shipping}")}" title="{l s='Previous'}" class="btn btn-lg btn-default">&laquo; {l s='Previous'}</a>
			{else}
				<a href="{$link->getPageLink('order', true, NULL, "step=1&multi-shipping={$multi_shipping}")}" title="{l s='Previous'}" class="btn btn-lg btn-default">&laquo; {l s='Previous'}</a>
			{/if}
		{else}
				<a href="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")}" title="{l s='Previous'}" class="btn btn-lg btn-default">&laquo; {l s='Previous'}</a>
		{/if}
		{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
			<input type="submit" name="processCarrier" value="{l s='Next'} &raquo;" class="btn btn-default btn-md" />
		{/if}
	</p>
</form>
{else}
	...
	<div class="leaveMessage">
		<h5>{l s='Leave a message'}</h5>
		<p class="message1">{l s='If you would like to add a comment about your order,'} <a class="showMessage">{l s=" click here."}</a></p>

		<p class="message2" >{l s='If you would like to add a comment about your order, please write it below.'}</p>

		<p class="carrierMessage"><textarea cols="120" rows="3" name="message" id="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea></p>
		<script type="text/javascript"> $('.carrierMessage, .message2').hide();
			$('.showMessage').bind('click',function (){
				$('.message1').fadeOut('slow',function (){
					$('.carrierMessage, .message2').fadeIn('slow');
				});
